...
- **vc**: The vertical coordinate. This is required to recognise upper-air variables over multiple levels e.g. `vc=pressure` for parameter `T`. For surface variables (e.g. T2m) or for a single level (e.g. T925), there is no need to set `vc`.
...
- *-params_list*: Which parameters for verify (default="T2m"). This should be a comma separated string of parameters, for example "T2m,S10m,T,S,T925". Generally these parameters must exist in the parameter list file, otherwise they will be skipped. The only excpetion to this is for parameters at a single upper-air level e.g. `T925`. In this case, if these parameters do not appear explicitly in the parameter list file, the scripting will use information from the corresponding upper-air parameters in the parameter list file e.g. `T925` will take its settings from `T`, `S500` from `S`, etc.. This does not happen if e.g. `T925` appears explicitly in the parameter list file. If `params_list` is not specified, it just defaults to "T2m". Note that for `params_list="All"` all parameters in the parameter list file are considered in the verification (this is NOT recommended in general).
